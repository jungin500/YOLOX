...
def main(exp, args, num_gpu):
        ...
        with torch.no_grad():
            ...
            for threshold in threshold_list:
                p_outputs = postprocess(
                    outputs, evaluator.num_classes, threshold, evaluator.nmsthre
                )
    ...
    for class_id, f1_scores in enumerate(perclass_f1_scores):
        ...
        f1_line, = plt.plot(threshold_list, f1_scores, color=color, linestyle=linestyle, label="F1 Score ({})".format(class_name))
        all_lines.append(f1_line)
        
    avg_max_threshold = np.mean(thresholds)
    max_score_line = plt.axvline(avg_max_threshold, color='r', linestyle='dashdot', label="Average max score ({:.02f})".format(avg_max_threshold))
