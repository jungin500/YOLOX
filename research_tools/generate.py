...
def make_parser():
    parser = argparse.ArgumentParser(
        "YOLOX COCO Annotation Generator",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-o",
                        "--output",
                        dest="output_path",
                        default="annotations_generated.json",
                        help="COCO-type Annotation output JSON filename")
    ...
    parser.add_argument("--tsize",
                        ...
                        type=int,
                        help="test img size")
    parser.add_argument("--generator-conf",
                        default=0.45,
                        type=float,
                        help="generator conf")
    parser.add_argument("--generator-iou",
                        default=0.3,
                        type=float,
                        help="generator iou threshold")
    parser.add_argument(
        "--fp16",
        dest="fp16",
        default=False,
    ...
    parser.add_argument("--rematch-thresh",
                        dest="rematch_thresh",
                        default=2,
                        type=int,
    ...
    parser.add_argument("--strategy",
                        ...
                        type=str,
                        choices=[
                            "naiive", "naiive-advanced", "iou", "scales",
                            "scales-simple"
                        ],
...
def main(exp, args, num_gpu):
    if args.seed is not None:
        ...
        print(
            "You're getting ready for the paper! be sure to set seed argument."
        )
        sys.exit(0)
    ...
    for conf_thresh_idx, conf_thresh in enumerate(conf_thresh_space):
        logger.info("Running confidence threshold {} ({}/{})".format(
            conf_thresh, conf_thresh_idx + 1, len(conf_thresh_space)))
        ...
        if args.image_ids:
            ...
            visualize(
                ...
                image_title="{}, conf={} nms={} g_conf={} g_iou={} rm_thresh={}"
                .format(args.strategy, args.conf, args.nms,
                        args.generator_conf, args.generator_iou,
                        args.rematch_thresh))
            ...
            if args.conf_eval or args.eval:
                ap5095, ap50, ap_s, ap_m, ap_l, AP_perclass, AR_perclass, AP50_perclass = eval_coco(
                    trainval_json=os.path.join(exp.data_dir, 'annotations',
                                               exp.train_ann),
    ...
    if args.conf_eval or args.eval:
        plot_result(fig_savedir, conf_thresh_space, all_ap5095, all_ap50,
                    all_ap_s, all_ap_m, all_ap_l, all_AP_perclass,
                    all_AR_perclass, all_AP50_perclass)
    logger.info("Done generating annotations, exiting ...")


def plot_result(fig_savedir, conf_thresh_space, all_ap5095, all_ap50, all_ap_s,
                all_ap_m, all_ap_l, all_AP_perclass, all_AR_perclass,
                all_AP50_perclass):
    # 2nd Index 접근을 위한 변환
    ...
    plt.title("AP5095 Score by Detection Threshold (NMS={:.02f})".format(
        exp.nmsthre))
    ...
    for idx, class_name in enumerate(CLASSES_COLOR_MAP.keys()):
        ...
        ap_line, = plt.plot(conf_thresh_space,
                            all_AP_perclass[:, idx],
                            color=color,
                            linestyle='dashed',
                            label=class_name)
    ...
    all_ap_line, = plt.plot(conf_thresh_space,
                            all_ap5095,
                            color="red",
                            linestyle='dashed',
                            label="AP")
    avg_max_threshold = float(np.mean(thresholds))
    max_score_line = plt.axvline(
        avg_max_threshold,
        color='r',
        linestyle='dashdot',
        label="Max score ({:.02f})".format(avg_max_threshold))
    plt.legend(handles=[*lines, all_ap_line, max_score_line],
               loc='upper right')
    ...
    plt.savefig(os.path.join(fig_savedir, "ap5095.svg"),
                dpi=600,
                transparent=True)

    # AP50 Per-class and All-class
    plt.figure(figsize=(8, 4))
    plt.title("AP50 Score by Detection Threshold (NMS={:.02f})".format(
        exp.nmsthre))
    ...
    plt.xticks(np.arange(0, 1, step=0.1))
    plt.yticks(np.arange(0.1, 1, step=0.1))
    plt.savefig(os.path.join(fig_savedir, "ap50.svg"),
                dpi=600,
                transparent=True)
...
def generate(exp,
             ...
             scales,
             reduce_master_rank=0):
    generator = build_generator(exp=exp,
                                model=model,
                                args=args,
                                is_distributed=is_distributed,
                                oneshot_image_ids=oneshot_image_ids,
    ...
    if is_distributed:
        ...
        coco_result = reduce_coco_result(rank=rank,
...
def build_generator(exp, model, args, is_distributed, oneshot_image_ids,
                    scales):
    # Generate annotation with given model
    if args.strategy == 'naiive':
        print("[naiive] Generating using Confidence {:.2f} and NMS-IoU {:.2f}".
              format(args.conf, args.nms))
        assert '--scales' not in ' '.join(
            sys.argv), "scales should not be applied on current strategy."
        assert '--generator-conf' not in ' '.join(
            sys.argv
        ), "generator confidence should not be applied on current strategy."
        assert '--generator-iou' not in ' '.join(
            sys.argv
        ), "generator iou should not be applied on current strategy."
        generator = NaiiveGenerator(exp=exp,
                                    model=model,
                                    device=args.device,
                                    is_distributed=is_distributed,
        ...
        perclass_conf_ious = [
            float(v) for v in args.perclass_conf_ious.split(',')
        ]
        assert len(
            perclass_conf_ious
        ) % 2 == 0, "--perclass-conf-ious only accepts a pair of conf_thresh, iou_thresh."
        perclass_conf_ious = list(
            zip(perclass_conf_ious[::2], perclass_conf_ious[1::2]))
        assert len(
            perclass_conf_ious
        ) == exp.num_classes, "Provided perclass_conf_ious doesn't match count of classes"

        print(
            "[naiive-advanced] Generating using following Confidence, NMS-IoU parameters"
        )
        for class_id, (conf_thresh,
                       iou_thresh) in enumerate(perclass_conf_ious):
            print(
                "[naiive-advanced] Class {}: conf_thresh {:.02f} iou_thresh {:.02f}"
                .format(class_id, conf_thresh, iou_thresh))

        assert '--nms' not in ' '.join(
            sys.argv), "--nms, --conf should be ignored."
        assert '--conf' not in ' '.join(
            sys.argv), "--nms, --conf should be ignored."
        assert '--scales' not in ' '.join(
            sys.argv), "scales should not be applied on current strategy."
        assert '--generator-conf' not in ' '.join(
        ...
        print("[iou] Generating using Confidence {:.2f} and NMS-IoU {:.2f}".
              format(args.conf, args.nms))
        print(
            "[iou] and Generating using Confidence {:.2f} and IoU Thresh {:.2f}"
            .format(args.generator_conf, args.generator_iou))
        assert '--scales' not in ' '.join(
            sys.argv), "scales should not be applied on current strategy."
        assert args.eval != True, "IOUStrategy depends on original dataset"
        generator = IOUGenerator(exp=exp,
                                 model=model,
        ...
        print("[scales] Inferring using Confidence {:.2f} and NMS-IoU {:.2f}".
              format(args.conf, args.nms))
        print(
            "[scales] and Generating using Confidence {:.2f} and IoU Thresh {:.2f}"
            .format(args.generator_conf, args.generator_iou))
        print("[scales] Scale list: {}".format(str(scales)))
        assert args.eval != True, "MultiscaleGenerator depends on original dataset"
        generator = MultiscaleGenerator(exp=exp,
                                        model=model,
                                        scales=scales,
                                        conf=args.generator_conf,
                                        iou_thresh=args.generator_iou,
        ...
        print(
            "[scales-simple] Inferring using Confidence {:.2f} and NMS-IoU {:.2f}"
            .format(args.conf, args.nms))
        ...
        raise NotImplementedError("Strategy type {} not implemented".format(
            args.strategy))
...
def reduce_coco_result(rank, world_size, output_path, coco_result, to_rank=0):
    ...
    if rank == to_rank:
        ...
        for idx, annotation in enumerate(all_annotations):
            ...
            assert len(
                annotation["images"]) > 0, "Empty images in file {}".format(
                    json_filename)
            assert len(
