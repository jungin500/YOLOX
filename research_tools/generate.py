...
def make_parser():
    ...
    parser.add_argument("--rematch-thresh", dest="rematch_thresh", default=2, type=int, help="Rematch threshold")
    ...
    parser.add_argument("--perclass-conf-ious",
        ...
        if args.image_ids:
            ...
            visualize(exp=exp,
                      ...
                      image_title="{}, conf={} nms={} g_conf={} g_iou={} rm_thresh={}".format(
                          args.strategy, args.conf, args.nms, args.generator_conf, args.generator_iou,
                          args.rematch_thresh))
...
def plot_result(exp, fig_savedir, conf_thresh_space, all_ap5095, all_ap50, all_ap_s, all_ap_m, all_ap_l, all_AP_perclass,
    ...
    # 결과를 가지고 Plot을 진행한다.
    CLASSES_COLOR_MAP = {"helmet_on": "#6EF018", "helmet_off": "#18A5F0", "belt_on": "#E718F0", "belt_off": "#F08C0C"}
    ...
    all_ap_line, = plt.plot(conf_thresh_space, all_ap5095, color="red", linestyle='dashed', label="AP")
    ...
    all_ap_line, = plt.plot(conf_thresh_space, all_ap50, color="red", linestyle='dashed', label="AP50")
    avg_max_threshold = float(np.mean(thresholds))
    max_score_line = plt.axvline(avg_max_threshold,
                                 color='r',
                                 linestyle='dashdot',
...
def build_generator(exp, model, args, is_distributed, oneshot_image_ids, scales):
    ...
    if args.strategy == 'naiive':
        ...
        assert '--generator-conf' not in ' '.join(
            sys.argv), "generator confidence should not be applied on current strategy."
        ...
        for class_id, (conf_thresh, iou_thresh) in enumerate(perclass_conf_ious):
            print("[naiive-advanced] Class {}: conf_thresh {:.02f} iou_thresh {:.02f}".format(
                class_id, conf_thresh, iou_thresh))
        ...
        generator = NaiiveAdvancedGenerator(exp=exp,
                                            model=model,
                                            device=args.device,
                                            is_distributed=is_distributed,
                                            batch_size=args.batch_size,
        ...
        print("[iou] and Generating using Confidence {:.2f} and IoU Thresh {:.2f}".format(
        ...
        print("[scales] and Generating using Confidence {:.2f} and IoU Thresh {:.2f}".format(
            args.generator_conf, args.generator_iou))
        ...
        generator = SimpleMultiscaleGenerator(exp=exp,
                                              model=model,
                                              scales=scales,
                                              conf=args.generator_conf,
                                              device=args.device,
