...
def make_parser():
    ...
    parser.add_argument("--small", default=None, action='store_true', help="Small visualization window (640x360)")
    parser.add_argument("--seed", default=None, type=int, help="Shuffle seed for displaying same image")
    parser.add_argument("--display", default=False, action='store_true', help="Display result image")
    ...
    parser.add_argument("--image-root",
                        default="parent",
                        type=str,
                        help="Image root folder of COCO annotation (default: ../val2017 of coco_annotation file)")
    parser.add_argument("coco_annotation", help="COCO annotation filename", default=None)
    parser.add_argument("ids", default=None, help="exclusive image id name to display", nargs=argparse.REMAINDER)
    return parser
...
def main(args):
    ...
    logger.info("Loaded {} images and {} annootations".format(len(annotation['images']),
    ...
    if len(fs_image_id_set - json_image_id_set) == len(fs_image_id_set):
        # 겹치는 구간이 없음
        logger.error("Filesystem has {} image set and JSON annotation has {} image set!".format(
            len(fs_image_id_set), len(json_image_id_set)))
    ...
    for image_path in image_list:
        ...
        if args.image_title:
            ...
            cv2.putText(image, args.image_title, (px, py + txt_middle_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (255, 255, 255), 2)

            # Display image name
            prev_height = txt_middle_pos + py * 2
            (txt_width, txt_middle_pos), _ = cv2.getTextSize(image_id, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
            cv2.rectangle(image, (0, prev_height), (txt_width + px * 2, prev_height + txt_middle_pos + py * 2),
                          (0, 0, 0), -1)
            cv2.putText(image, image_id, (px, prev_height + py + txt_middle_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (255, 255, 255), 2)
        else:
            # Display image name
        ...
        if image_id in bbox_map:
            ...
            for item in bbox_map[image_id]:
                ...
                cv2.rectangle(image, (xmin, ymin), (xmax, ymax), colormap[item['category_id']], 2)
                ...
                cv2.putText(image, class_name_text, (xmin + 4, ydraw), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2)
                # real text
                cv2.putText(image, class_name_text, (xmin + 3, ydraw - 1), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
